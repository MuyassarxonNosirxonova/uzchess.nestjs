...
export class PurchasedCourseController {
  ...
  async purchase(@Param('id', ParseIntPipe) courseId: number, @Req() req: Request) {
    // @ts-ignore
    const userId = req.user.id;
    return await this.cmdBus.execute(new PurchaseCourseCommand(userId, courseId));
  }

  @Get('my-courses')
  @Roles(Role.User, Role.Admin)
  @ApiOkResponse({ type: PaginatedResultDto(GetMyCoursesResponse) })
  async getMyCourses(@Query() filters: GetMyCoursesRequest, @Req() req: Request) {
    // @ts-ignore
    const userId = req.user.id;
