...
export class CourseLessonController {
  ...
  async findOne(@Param('id', ParseIntPipe) id: number, @Req() req: Request) {
    ...
    return await this.queryBus.execute(new GetOneCourseLessonQuery(id, userId, isAdmin));
  }
  ...
  async create(
    ...
    return await this.cmdBus.execute(payload.toCommand(courseId, video, files.thumbnail?.[0]));
  }
  ...
  async update(
    ...
    return await this.cmdBus.execute(payload.toCommand(id, files.video?.[0], files.thumbnail?.[0]));
  }
