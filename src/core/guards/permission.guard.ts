...
export class PermissionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext) {
    ...
    const allowedPermissions = await permissions.findBy({userPermissions: {userId: req.user.id, isAllowed: true}});
    const deniedPermissions = await permissions.findBy({userPermissions: {userId: req.user.id, isAllowed: false}});
    const rolePermissions = await permissions.findBy({
